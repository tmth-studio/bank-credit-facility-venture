# A business bank's credit facility line — challenge one (R1) record v2 · Path C, mode 3 · 18 September 2026

> **v2 supersedes v1 on the diagnosis, not the design.** The insider corrected the baseline on 18 September 2026: the bank already operates pre-approved limits at scale (count a red field), is digitising fulfilment end to end, and is extending pre-approval to newly joined clients. The decision has therefore already been made cheap for a large part of the base. The cost and the exclusion that remain sit in what happens *after* the decision — the fixed, secured, scheduled, reviewed and collected exposure. The critical limiting operation moves from the making zone to the delivering zone; the six design claims stand as written in v1; the financial check is re-cut against what the bank's own roadmap leaves standing. v1 is retained as the record of the first diagnosis.
...
**The baseline, corrected by the insider (18 September 2026).** The bank already holds `[clients with a pre-approved limit]` — a limit computed on account conduct and bureau data and shown before any application — and the market does the same (public: the high-street banks' in-app pre-assessed business borrowing; Tide, Starling and Monzo for newer accounts). Its stated direction is fully digital fulfilment of those limits and extension of pre-approval to newly joined clients. INFERENCE: for that part of the base the decision has already been substituted — account conduct in place of applicant evidence — and made cheap. What the pre-approved form keeps, for every facility: an acceptance step with a check attached; a limit assessed at a point in time and refreshed on a cycle; security above a threshold (⚠ £25–50k, judged from public terms); a fixed schedule for a loan or a clear-down expectation for an overdraft; an annual review; and a collections operation when the schedule is missed. Section SR2 diagnoses this form — the roadmap form, not the older application form — because it is the form the bank will be running when this venture would launch.

**Research firewall.** The pre-approved limit and its digital fulfilment are the baseline, not an analogue. Section SR5 tests the difference.

---

## SR2 — Critical limiting operation

*Method note.* Mode 3 diagnoses the incumbent's form for its current borrowers at its current volume. The form diagnosed is the **roadmap form** — pre-approved limits, digitally fulfilled — because that is the form the venture would meet. The within-form cost hunt runs on the eight-zone map at the bank's volume: `[average drawn SME balance £]`, public proxy ⚠ £3.4–5.7bn (AOM header).

**Where the cost sits in the roadmap form — per £1,000 of drawn balance a year, blended small-business book, all ⚠ (AOM 3d and 6, re-cut):**

| Cost line | ⚠ £ per £1,000 | Zone | Fixed per facility? |
|---|---|---|---|
| Funding — deposits | 15–30 | Delivering | no — scales with balance |
| Expected loss on a pre-approved, part-secured pool | 10–25 | Delivering | no |
| Regulatory capital at the cost of equity | 10–14 | Delivering | no |
| The decision — pre-approval in batch on conduct and bureau data, plus the acceptance check at fulfilment | 1–3 | Making | near-zero — this is what the roadmap has removed |
| The decision for a client with no conduct history — applicant evidence or bureau only (new joiners; the base without a pre-approved limit, `[accounts without a pre-approved limit]`) | 7–31 on that segment | Making | yes — ⚠ £500–1,500 per person-decision |
| Security — taking, perfecting, releasing a guarantee or charge above the threshold | 2–5 | Delivering | yes — ⚠ £150–400 per facility, judged |
| Annual review of each facility | 8–21 | Delivering | yes — ⚠ £200–500 a year |
| Collections and recoveries (FCA SME collections review 2022, 11 banks) | 5–15 | Delivering | yes per default case |
| Relationship management allocated to lending | 5–20 | Selling | yes per client above £2m |
| **Roadmap-form floor** | **⚠ 63–164 (of which 57–133 on the pre-approved segment)** | | |

INFERENCE: on the pre-approved segment the making cost is ⚠ 1–3; the fixed-per-facility cost that remains is security, review, collections and the relationship — ⚠ 20–61 per £1,000 at the blended ticket. **Those lines do not scale with the size of the facility.** On a £10,000 facility, review and security alone are ⚠ £350–900 a year — ⚠ £35–90 per £1,000, at or above the whole price the borrower pays today. The roadmap has moved the exclusion, not removed it: the bank can now *decide* on the small client cheaply; it still cannot afford to *hold* the small facility.

**The operation, stated as a verb.** *Holding each facility as a fixed exposure — secured above a threshold, repaid to a schedule, reviewed each year and collected when the schedule is missed — against the borrower's balance sheet.* Not "the credit function" or "collections" — those are the resources. The operation that forces them to be spent is the holding of a fixed, secured, scheduled exposure.

**Why the cost is inherent to the form.** Four named mechanisms:

1. **A fixed exposure needs monitoring, and monitoring is the bank's cost.** FACT: a bank exists because it monitors borrowers on depositors' behalf more cheaply than they could, and the cost of monitoring is the cost of holding the exposure (Diamond, *Review of Economic Studies*, 1984). INFERENCE: an exposure that does not move with the evidence must be re-checked by hand on a cycle; the annual review is the form's rational answer to a fixed limit against a moving business.
2. **Security is the form's answer to not being able to see the cash.** FACT: collateral substitutes for information where the lender cannot observe the borrower's cash flows (Berger and Udell, 2006; Stiglitz and Weiss, 1981 on collateral and rationing). INFERENCE: a lender that sees every receipt does not need the charge; a lender that holds a fixed exposure to a schedule does.
3. **A schedule creates the default event, and the default event creates collections.** FACT: the FCA's 2022 review of SME collections found 11 banks running distinct forbearance and recovery functions with case-by-case handling. INFERENCE: the default event exists because repayment is decoupled from receipts; couple them and the event has no cause.
4. **Fixed per-facility costs set a minimum ticket.** FACT: the transaction cost of a small facility is comparable to that of a large one (Mills and McCarthy, 2014). INFERENCE: the roadmap cut the decision's fixed cost; security, review and collections keep their own, so the minimum ticket survives and the smallest business stays refused or under-served.

**Essential and tightly coupled.** Remove the holding of a fixed, secured, scheduled exposure and the pre-approved form has no product: the pre-approved limit *is* an offer to hold that exposure. Security, review and collections all hang from it. ✓

**Cost threshold, for current borrowers.** The price paid today — ⚠ 7.5–12% all-in — covers the roadmap floor only across the book; on its smallest facilities the delivering lines alone exceed the price. In mode 3 there is no cost-to-value barrier for the current borrower; the threshold reported is the drop and the population the form still refuses (PCO section 3: about 1.5m businesses self-funding or deterred, ⚠ UNVALIDATED).

**CLO statement (registry, Phase 1): a process step, not a market condition.** ✓ Zone: Delivering.

**Secondary cost, recorded.** Funding and capital — ⚠ 25–44 of the floor — are the balance-sheet cost of holding exposure of any shape. Same in every form that lends; not the limiting operation. Held for R3 by number. The making cost for the segment with no conduct history (⚠ 7–31) is secondary in the roadmap form and is answered in section SR5 (new joiners).

---

## SR3 — Workaround theory of change (CMO)

- **Class of problem.** A fixed exposure held against a moving business: because the exposure does not follow the evidence, it must be secured, re-checked on a cycle and collected when the schedule breaks — each a fixed cost per object that never amortises on a small object. Secondary: information asymmetry at the point of decision, already largely answered by the roadmap for clients with conduct history.
- **Theory.** Diamond (1984): the bank's cost is delegated monitoring; an exposure that moves with observed cash flow is monitored by the flow itself, so the review function has no work. Stiglitz and Weiss (1981): rationing and collateral are the rational responses to not knowing; remove the not-knowing and both lose their cause. Berger and Udell (2006): the lending technology follows the information available — transaction lending on hard information replaces relationship lending where hard information exists. FinRegLab, *The Use of Cash-Flow Data in Underwriting Credit* (2019) and *Sharpening the Focus* (2025, 38,000 small-business loans from two lenders, 2015–2024): cash-flow variables from bank-account data predict default across every segment tested, independently of the owner's credit score, with the largest gains for young businesses and low-score owners. Mills and McCarthy (2014): the fixed cost of the decision is the exclusion mechanism. Coase (1937): information inside the firm is cheaper than information bought across a market — the bank that holds the account already holds the flow.
...
- **Outcome (a state).** A business with an account draws working money against a limit that follows its flow and repays it from its receipts; the bank holds no fixed exposure, takes no security, runs no annual review and no collections operation on this book. Substitution test: "we will use cash-flow data in underwriting" is an activity inside the old form; the outcome is the state in which no application is assessed.
- **Current state, observable.** FACT: success 47%, falling to 39% for the smallest; 22% of SMEs put in personal money because they had to; 5% deterred from applying (S2). Insider-confirmed: pre-approved limits at scale, fulfilment being digitised, extension to new joiners in progress; every approved facility is still secured above a threshold, held to a schedule, reviewed each year and collected on default.
...
- **A′ — the bank's own roadmap: pre-approval in batch, digital fulfilment, extension to new joiners:** an improvement inside the old form. It removes the decision's cost for clients with conduct history and leaves security, schedule, review and collections in place — the delivering-zone operation this record names. It is the baseline the design is measured against, and the design is built to sit on top of it, not beside it.
...
```
CLO GATE — R1
cloZone: Delivering   (v2 — moved from Making after the insider corrected the baseline)
cloName: holding each facility as a fixed, secured, scheduled exposure that is reviewed and collected
cloIsOperationNotResource: true   (the operation is holding a fixed exposure to a schedule against the borrower's balance sheet; security administrators, reviewers and collectors are what it consumes)
cloStatedAsVerb: holding each facility as a fixed exposure — secured above a threshold, repaid to a schedule, reviewed each year and collected when the schedule is missed — against the borrower's balance sheet
cloIsEssential: true   (remove it and the pre-approved form has no product — the pre-approved limit is an offer to hold exactly that exposure)
cloEliminatesNotReduces: true   (restructured: no fixed exposure is held; the limit follows the flow, the balance repays from receipts, no security, no review, no collections; the residue — a rule under model-risk review and exception cells for accounts whose flow stops — is named and costed in the AOM)
...
newBFFRequired: true   (the roadmap form decides cheaply and then holds a fixed, secured, scheduled exposure; the seed holds a variable exposure that follows the flow and repays from receipts, with no security, review or collections; the object sold, the risk held and the people employed all change)
cloAuditFlag: false
```
...
**New joiners — the desk answer (v2).** The bank's roadmap and this design meet the same wall at a newly joined client: no flow history at this bank. FACT: under the CMA's Retail Banking Market Investigation Order 2017 the nine largest current-account providers must give a regulated account-information provider read access to a customer's transaction history with the customer's consent, and the history available is the account's, typically the full period the provider retains (commonly 12 months or more online). FACT: 22% of SMEs used open banking data in a finance application in 2024 (Open Banking Ltd impact reports, 2024–25, ⚠ figure to be checked against the report edition). INFERENCE: the flow exists; it is held at the previous bank; it is available on day one. The limit can be computed from the previous bank's flow before the first receipt lands here, and the receipts share starts on the first receipt. The flow gate (⚠ six months) is therefore met at joining for any client switching from a mandated provider, and the "start-up" exclusion narrows to businesses with no account history anywhere — the 314,000 a year (S1) who are new to trading, carried to C2 by number. This answer serves the bank's roadmap and this venture identically, and it places the standing limit at the moment of joining, which is where a switching cost is born (R8 by number).
...
|---|---|---|---|
| Made | a credit decision on financials, rating and security (v1 entry); pre-approved in batch on conduct, digitally fulfilled (v2 baseline) | a limit computed daily from the flow; no acceptance event | SR4 |
| Sold | on application (v1); shown pre-approved, accepted in the app (v2) | never accepted; live in the account | SR4 |
...
**The drop — measured against the roadmap form, not the old application form.** Roadmap-form floor on the pre-approved segment ⚠ 57–133 (SR2). New form ⚠ 54–105. **The delivering lines that are fixed per facility — security, review, collections, relationship — fall from ⚠ 20–61 to ⚠ 5–13 (the exception cells and the rule); a drop of ⚠ 75–80% on those lines.** Expected loss rises from ⚠ 10–25 to ⚠ 20–40 because the pool is no longer protected by security and is broader than the pre-approved set. Capital rises by ⚠ 1.6–3.4 for the undrawn limits. **Net floor at the blended book: down ⚠ 3 at the low corner and ⚠ 28 at the high corner — ⚠ 5–21%.** Smaller than v1's ⚠ 10–32%, because the roadmap has already taken the decision's cost out. The form's real change is unchanged: the floor per £1,000 no longer rises as the facility shrinks, which is what lets the bank hold the small facility the roadmap can now approve but still cannot afford to hold.

**Margin of safety against the price paid today (non-gating in mode 3, RD-024 (e)).** Price the borrower pays today: ⚠ £75–120 per £1,000 (FACT: 6.18–7.00% effective on new SME loans through 2025; overdrafts and small unsecured ⚠ 10–15%; fees ⚠ 1–2% equivalent; blended ⚠ 7.5–12%). By the script (`fit_margin.py`, ceiling = the price paid with the fee element discounted by a twentieth, £71.25–114): **−32.1% at the worst corner; 111.1% at the best.** By hand on the undiscounted price: (75 − 105) ÷ 105 = −28.6%; best corner (120 − 54) ÷ 54 = 122%. At the centre of both bands (floor 79.5, price 97.5): 22.6% — BORDERLINE band. The roadmap form on the same unit sits at (75 − 133) ÷ 133 = −44% at its worst corner and (120 − 57) ÷ 57 = 111% at its best — the same shape, because both forms carry the same funding and capital and differ in where the rest sits. **The check reports a margin; it does not pass or fail the design.** The script's output is in `sme-fintech-models/bank-credit-facility-fit-verifier-r1-2026-09-18.md`.

**What the margin says.** The worst corner is negative in both forms. In the roadmap form the negative corner is the small facility, which the delivering lines make uneconomic to hold. In the new form the negative corner is the high-loss pool, which the rule must avoid. **The design has moved the binding constraint from the size of the facility to the loss rate on the pool.** That is the finding of this run, and it is reported unsoftened.
...
**R1 failure test (RD-024 (d), the gate in mode 3) — v2.** Could the bank adopt this without changing its architecture? No — and the corrected baseline makes the test sharper, not weaker. The bank's roadmap already makes the decision cheap and digital; what it leaves standing is the architecture of *holding*: a fixed limit against a balance sheet, security above a threshold, a schedule, an annual review, a collections function, and the delegated authorities and credit policy that govern all of them. The seed removes the fixed exposure and with it security, review and collections; it replaces the borrower's balance sheet as the risk object with the continuity of the borrower's receipts; it makes the exposure continuously variable, which changes the capital treatment and the model-risk governance; and it makes the account, not the facility, the product. Risk governance, the people employed, the risk object and the object sold all change. **A new form is required. Verdict on the qualitative test: venture, not improvement — on the delivering side of the form.** The honest tension, restated: the bank could build the limit engine as a feature (it largely has). It could not run the receipts share, drop security, and close review and collections on this book without re-architecting credit risk — and that is the venture.

**Principal's review, 18 September 2026 (carried from v1).** Tom asked whether banks already make the facility a standing property of the account. Answer on record: the bank already makes the *decision* cheaply (pre-assessed limits in the app, priced on account conduct); it has not *removed* the decision — an acceptance step with a check, a point-in-time assessment refreshed on a cycle, security above a threshold, a schedule or a clear-down expectation, and annual review and collections all remain. Non-banks (Square Capital, PayPal Working Capital, Shopify Capital, iwoca) do the receipts-share repayment, on their own transaction data only, at ⚠ 15–30% equivalent rates. Falsification stated to Tom: if inside the bank the pre-assessed limit is drawable with no acceptance step, recomputed from the flow, and unsecured, unscheduled and unreviewed, the design is the bank's existing form and C1 fails as a venture. Tom's response: "think you are right." The failure-test verdict stands; the falsification remains the insider's to apply if any of the five operations turns out to be absent already.

---

## Carried items (VA-82)

- Forward-referenced: the limit for a business with no account history anywhere (new to trading) — C2 by number; the switching cost born at joining — R8 by number; the price and its presentation — C6 by number; who brings the customer to the limit — C7 by number; the balance-sheet shape of a self-liquidating exposure — R3 by number; the customer's conviction that the limit is real — C4 by number.
...
INFERENCE. The operating floor per £1,000 no longer rises as the facility shrinks, so the size gate has no cause. What still excludes is the flow gate: a business with less than ⚠ six months of account history at any mandated provider, or one whose receipts arrive as cash outside any bank's payment systems. The 314,000 new-to-trading businesses a year (S1) and the cash trades are on the far side of it; a switcher with history elsewhere is on the near side (open banking, section SR5). The 1.25m self-funders and 285,000 deterred (PCO section 3, ⚠ UNVALIDATED) who hold an account with flow are on the near side. Sized at R2, not claimed here.
...
| v2 | 18 September 2026 | Insider corrected the baseline (pre-approved limits at scale, digital fulfilment, extension to new joiners; count a red field). CLO restated in the delivering zone — holding a fixed, secured, scheduled exposure. Roadmap form costed and used as the comparison floor; drop re-cut to ⚠ 5–21% at the blended book. New-joiner desk answer (open banking). Failure test restated. Design claims unchanged. |
