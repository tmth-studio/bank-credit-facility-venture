# Business bank credit facility line — customer transformation map (CTM), v3, at C3

**Venture:** the credit facility line of a large UK SME bank (Case B — term loan and overdraft), Path C mode 3 — see `../bank-credit-facility-c3-v1-2026-09-21.md`. **v3 written at R3** (21 Sep 2026), runner-drafted under the propose-and-confirm rule. **v2** (`bank-credit-facility-ctm-v2-2026-09-21.md`) and **v1** are retained unchanged as the C2 and C1 records. **Update mode:** only the rows R3 adds or changes are new (marked *C3*); every other row is carried from v2 with its trace. R3 changes the Making stream's release order and adds one internal track; the Payment product's structure stays `[REQUIRES C6]`. **Customer at C2 (selected at R2):** the established UK SME with observable account flow that currently pays a standard-tariff overdraft at 15–30% EAR for recurrent working-capital draws. **Naming note:** the challenge skill names the model file `bank-credit-facility-ctm-model-at-C3.yaml` and generates the views from it; this venture's pattern (C1) is a document, so this file is the record and the model file stays a build item — the runner's call, carried in the C3 record. Every state change is traced to a requirement by number; anything a later requirement owns is flagged `[REQUIRES C[N]+]`, not designed. The bank is not named. Every figure only the bank holds is a red field `⚠ [description]`. Every ⚠ figure is Tier 4 until the insider corrects it.
...
| CP-1 | Communications | the limit statement — the standing limit and the receipts share, visible in the account before any need arises | C1 (v2 amended at C2: the statement also shows, once a year, what the customer paid on the overdraft in the year before migration and what the limit cost in the year since; **v3 at C3: during the scaling path the statement appears at the account's release, not at its eligibility — a path rule, not a product change**) | the "aware" state (C1); the "I can see the saving" state (C2); the "my limit appeared" state (C3, path only) |
...
### Phase 3 — Paying (the price is C6; R3 leaves the customer's paying rows unchanged — the working capital structure R3 sets is the bank's, on the release track below)
...
| **the rule is back-tested on the bank's own history before any release** | **WP-5** | **the limit engine team** | **C3 — Stage 0; the record stays as validation evidence; the gate falls away** |
...
Every C1, C2 and C3 transition carries a component or a flag. No transition is carried by a component that a later requirement has not yet designed: PC-1 is a condition on the Payment product, not the product.

**Component set at C3 (for the margin-of-safety check, VA-74):** WP-1, WP-2, WP-3, WP-4, WP-5, CP-1 (amended at C2; the release state added at C3), PC-1 (condition). The path-only rows (WP-5's release states) exist during Stages 0–2 and not at scale; the at-scale journey is the v2 journey unchanged.
