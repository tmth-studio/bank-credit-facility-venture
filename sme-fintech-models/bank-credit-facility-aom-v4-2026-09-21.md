# Business bank credit facility line — activity and operating model (AOM), v4, at C4

**Venture:** the credit facility line of a large UK SME bank (Case B), Path C mode 3 — see `../bank-credit-facility-c4-v1-2026-09-21.md`. **v4 written at R4** (21 Sep 2026), runner-drafted under the propose-and-confirm rule. **v3**, **v2** and **v1** are retained unchanged as the C3, C2 and C1 records. **Update mode:** only the rows R4 adds or changes are new (marked *C4*, in bold); every other row is carried from v3 with its trace and its bold removed. R4 adds the Selling stream's only activities — the limit history, its placement, its sign-off and the queries it produces. It also restates two inputs as bands (the productive hours, the surplus fraction) that verification faulted at F1. The conventional Selling activities (marketing, outreach) are named in 3d as removed. The model file (`bank-credit-facility-aom-model-at-C4.yaml`) is a build item carried from C1; this document is the record until it exists.

**Defined terms.** As in the CTM v2: *flow*, *standing limit*, *draw*, *receipts share*, *account line*, *standard tariff*, *migration*, *price condition*. *Exception cell* — the smallest operating unit of people in the form: the team that handles accounts whose flow has stopped, whose limit rule has been overridden by a fraud or conduct signal, or whose holder has declined migration or queried the saving statement. New at C3, as in the CTM v3: *shadow limit*, *back-test*, *release*, *overlay*, *trial band*, *replica band*. *Stage* — one step of the release order: Stage 0 the back-test, Stage 1 the migrated accounts, Stage 2 the replicas, Stage 3 at scale. New at C4, as in the CTM v4: *shortfall*, *limit history*, *placement*. *Productive hours* — the hours a year one handler spends on cases, after leave, public holidays, sickness and non-case time.
...
floors:
  f.cell_min_people: 13 handlers + 1 lead (band 12–14) — reason: the C4 row adds ⚠ 830–2,500 hours a year to the 17,000 of v3; at the high corner (19,500 hours) and 1,400–1,600 productive hours the cell needs 12.2–13.9 handlers; 13 is the figure used (VA-91)
```
...
| The account line (Case A) | holds the flow and the balance; the ledger the limit engine reads; **produces the saving statement (C2)** | internal dependency — not designed here; **stores each day's limit, renders the limit history and places it at the shortfall (C4)** |
...
| The exception cell | the people who act when the flow stops, a signal fires, a customer disputes, **a customer declines migration or queries the saving statement (C2)** | LMU; **answers a customer who asks why the limit on a day was what it was (C4)** |
...
Per drawn account, the automated activities cost ⚠ £2–5 a year on a ⚠ £15–25k balance — ⚠ £0.1–0.3 per £1,000 drawn, rounded up to £2–5 per £1,000 in the FIT model to hold a margin for the computing and payment cost the account line would allocate. The C2 rows add ⚠ £0.003 per £1,000 a year (the statement) and nothing else to the annual line; the rounding absorbs them. The C4 row adds ⚠ £0.01–0.07 per £1,000 a year; the computing line is carried as 2.01–5.07 so the addition is visible against the ⚠ £8 cap (C4 record, financial check).
...
| **Cell total, recurring** | | | **q.cell_drawn_accounts = 100,000 (q.replica_accounts = 333,000 with a limit)** | | | **⚠ 17,830–19,500 — the figure used is the high corner, 19,500 (C4)** |
...
**Capacity figure used throughout (VA-91): ⚠ 19,500 exception hours per cell a year (C4 — the high corner of 17,830–19,500); ⚠ 44,500 hours at scale (2.28 cells on ⚠ 228,000 drawn accounts).** One cell is ⚠ 13 handlers (band 12–14) at ⚠ 1,400–1,600 productive hours (18,200–20,800 hours against the 19,500) with a cell lead; the trainer is shared across cells. At the low corner of demand (17,830 hours) and the high corner of hours (1,600) the cell runs on 12 handlers with 1,370 hours of slack. At the high corner of demand and the low corner of hours (1,400) it needs 14. At scale: ⚠ 2.28 cells, ⚠ 30 handlers and 3 leads, ⚠ 34 people with the shared trainer. Every calculation in the C4 record uses 19,500 and 13.
...
| product, conduct and compliance — account terms, the CCA agreement under £25,000, Consumer Duty; the migration agreements and the saving statement's conduct standard (C2, +2 people); **the limit-history display as a financial promotion template — sign-off, yearly review, complaints (C4, +1 person: the conduct reviewer)** | product | **13** | **1.3–1.95** | fixed per HQ |
...
| **HQ running cost (the RC layer)** | | **⚠ 48** | **⚠ 7.3–12.95** | **C4: +1 person, +⚠ £0.1–0.15m = +⚠ 0.02–0.04 per £1,000** |
| exception cells (from 3b; counted in the PVC layer, not here) | money · information · product | **34** | **1.9–2.55** | q.drawn_accounts — **C4: 13 handlers per cell (band 12–14), 2.28 cells** |
| **All people in the form** | | **⚠ 82** | **⚠ 9.2–15.5** | ≈ ⚠ 0.16–0.46% of the drawn book, stated for scale only — no cost line is derived from it |
...
**Conventional additions used against the ⚠ £8 cap (RD-030):** R4 ⚠ 0.03–0.20; cumulative through R4 ⚠ 0.03–0.20; remaining at the high corner ⚠ 7.80.
...
| people | **13 handlers (band 12–14) + 1 lead + 1 trainer (shared across cells) — C4** |
| fixed cost a year | **£0.85–1.12m (13 handlers ⚠ £50–65k loaded; lead ⚠ £70–90k; trainer share ⚠ £20–30k; tooling and premises ⚠ £0.1–0.15m) — C4; was £0.8–1.05m at 12 handlers** |
...
| PVC — computing and payments | **2.01–5.07** | 3a, rounded up; the saving statement adds ⚠ £0.003; **the limit history adds ⚠ 0.01–0.07 (C4)** |
| PVC — exceptions and write-off handling | **3.00–8.09** | 3b: ⚠ £1.9–2.55m across 2.28 cells ÷ £3.4–5.7bn = £0.33–0.75, plus write-off administration and legal ⚠ £3–7; **the limit-query handlers add ⚠ 0.00–0.09 (C4)** |
| **PVC** | **40.01–83.16** | **C4: +0.01–0.16** |
| RC | **2.02–5.04** | 3c: ⚠ £7.3–12.95m ÷ £3.4–5.7bn = £1.28–3.81, carried at the C1 band **plus the conduct reviewer, ⚠ 0.02–0.04 (C4)** |
...
| **IC** | **12–17** | rounded |
| **Cost floor at C4 (PVC + RC + SC + IC)** | **54.03–106.20** | **C4 adds ⚠ 0.03–0.20 per £1,000 to the 54–106 of C3; carried at two decimals so the addition is visible against the ⚠ £8 cap** |
