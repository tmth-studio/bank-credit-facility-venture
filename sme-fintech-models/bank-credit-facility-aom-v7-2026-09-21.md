# Business bank credit facility line — activity and operating model (AOM), v7, at C7

**Venture:** the credit facility line of a large UK SME bank (Case B), Path C mode 3 — see `../bank-credit-facility-c7-v1-2026-09-21.md`. **v7 written at R7** (21 Sep 2026), runner-drafted under the propose-and-confirm rule. **v6**, **v5**, **v4**, **v3**, **v2** and **v1** are retained unchanged as the C6, C5, C4, C3, C2 and C1 records. **Update mode:** only the rows R7 adds or changes are new (marked *C7*, in bold); every other row is carried from v6 with its trace and its bold removed. R7 adds PP-1 — the published limit: the released standing limit carried as the account's pre-agreed credit line in the balance response the bank already serves to every account-information reader under the Open Banking standard, and the receipt splits as the transaction lines they are. The Selling stream gains one activity class (access) with no acquisition activity, no partner agreement and no fee; the operating model gains one internal holder (the bank's account-information team), two one-off build items in the SC layer and one exception row inside the C5 headroom. The floor moves by ⚠ 0.00–0.01 per £1,000 a year. Nothing in the form is removed or amended.
...
| **SC layer, one-off total** | | | | | **⚠ £0.72–12.25m** (back-test 0.3–0.8; migration 0.3–0.85; readings 0.045–0.135; carrying cost 0–10.4; the term drafting 0.015–0.03 (C5); the representative example 0.01–0.02 (C6); **the element build and the conformance test 0.05–0.10 (C7)**) | **÷ 5 years ÷ £3.4–5.7bn = £0.03–0.72 per £1,000 a year → 0–1 (section 6)** |
...
| PVC — exceptions and write-off handling | 3.00–8.13 | 3b: ⚠ £2.05–2.7m across 2.28 cells ÷ £3.4–5.7bn = £0.36–0.79, plus write-off administration and legal ⚠ £3–7; the limit-query handlers add ⚠ 0.00–0.09 (C4); the funding-query handlers add ⚠ 0.00–0.04 (C5); the charge queries add nothing (inside the C5 headroom, C6); **the display queries add nothing — inside the C5 headroom (C7)** |
...
| **IC** | **12–17** | rounded |
| **Cost floor at C7 (PVC + RC + SC + IC)** | **54.04–106.28** | **C7 adds ⚠ 0.00–0.01 per £1,000 to the 54.04–106.27 of C6; carried at two decimals so the addition is visible against the ⚠ £8 cap** |
