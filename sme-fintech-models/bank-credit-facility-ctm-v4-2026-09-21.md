...
Every C1, C2, C3 and C4 transition carries a component or a flag. No transition is carried by a component that a later requirement has not yet designed: PC-1 is a condition on the Payment product, not the product.

**Component set at C4 (for the margin-of-safety check, VA-74):** WP-1, WP-2, WP-3, WP-4, WP-5, CP-1 (amended at C2 and C4), **CP-2 (new at C4)**, PC-1 (condition). The path-only rows (WP-5's release states) exist during Stages 0–2 and not at scale; the at-scale journey is the v2 journey unchanged.
